...
class GameUI {
    ...
    update() {
        ...
        if (player.level >= 99) {
            this.xpText.textContent = `${formatNumber(player.xp)} (Max)`;
        } else {
            this.xpText.textContent = `${formatNumber(player.xp)}/${formatNumber(requiredXP)}`;
        }

        // Update stats display
        this.levelText.textContent = player.level;
        this.coinsText.textContent = formatNumber(player.coins);
        this.materialsText.textContent = formatNumber(player.materials);
    ...
    updateCharacterPanel() {
        ...
        const damageEl = document.getElementById('stat-damage-panel');
        const defenseEl = document.getElementById('stat-defense-panel');
        const hpEl = document.getElementById('stat-hp-panel');
        const speedEl = document.getElementById('stat-speed-panel');
        const critEl = document.getElementById('stat-crit-panel');

        if (damageEl) damageEl.textContent = player.stats.damage;
        if (defenseEl) defenseEl.textContent = player.stats.defense;
        if (hpEl) hpEl.textContent = player.stats.maxHp;
        if (speedEl) speedEl.textContent = player.stats.speed.toFixed(1);
        if (critEl) critEl.textContent = (player.stats.critChance * 100).toFixed(1) + '%';

        // Check for stat points
        // (Assuming you have logic here for stat points, keeping it as is or generic)
    }
    ...
    setupDebugButton() {
        ...
        if (levelSlider && levelInput && setLevelBtn) {
            // Sync Input -> Slider
            ...
            setLevelBtn.addEventListener('click', () => {
                ...
                if (newLevel >= 1 && newLevel <= 99) {
                    ...
                    // Reset XP to 0 for current level (or whatever appropriate)
                    ...
                    this.updateCharacterPanel();
                    this.updateDebugInfo();
                    this.showNotification(`Debug: Level set to ${newLevel}`, 'level-up');
    ...
    updateDebugInfo() {
        const player = this.game.player;
        document.getElementById('debug-coins').textContent = formatNumber(player.coins);
        document.getElementById('debug-materials').textContent = formatNumber(player.materials);
        document.getElementById('debug-level').textContent = player.level;
        document.getElementById('debug-xp').textContent = formatNumber(player.xp);
        ...
        const levelInput = document.getElementById('debug-level-input');
        const levelSlider = document.getElementById('debug-level-slider');
        if (levelInput && levelSlider) {
            levelInput.value = player.level;
            levelSlider.value = player.level;
        }
    }
